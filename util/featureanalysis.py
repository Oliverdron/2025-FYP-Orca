...
class FeatureAnalysis:
    ...
    def plot_feature_combinations(self, combinations_to_plot=None):
        ...
        if combinations_to_plot is None:
            combinations_to_plot = list(combinations(available_features, 2))
        ...
        for feat1, feat2 in combinations_to_plot:
            ...
            # Plot non-cancerous points first (if label exists)
            if 'label_binary' in self.df.columns:
                # Non-cancerous points
                non_cancer = self.df[self.df['label_binary'] == False]
                plt.scatter(non_cancer[feat1], non_cancer[feat2],
                           c='blue', alpha=0.6, s=30, label='Non-cancerous')
                
                # Cancerous points (with outline only)
                cancer = self.df[self.df['label_binary'] == True]
                plt.scatter(cancer[feat1], cancer[feat2],
                           facecolors='none', edgecolors='red',
                           linewidths=1.2, s=60, alpha=0.9, label='Cancerous')
            else:
                # If no labels, just plot all points
            ...
            plt.text(0.95, 0.05, f'r = {corr:.2f}', 
...
#             plt.scatter(self.df[feat1], self.df[feat2], 
#                        c='blue', alpha=0.8, label=feat1,s=20)
#             plt.scatter(self.df[feat2], self.df[feat1], 
#                        c='orange', alpha=0.8, label=feat2,s=20)
            
#             # Add cancer outlines if label_binary exists
#             if 'label_binary' in self.df.columns:
#                 # Cancerous cases (with red outline)
...
#                 # # Non-cancerous cases (no outline, just keep original colors)
#                 # non_cancer_mask = self.df['label_binary'] == False
#                 # plt.scatter(self.df[feat1][non_cancer_mask], self.df[feat2][non_cancer_mask],
#                 #            c='blue', alpha=0.4, label='Non-cancerous (Feat1)')
#                 # plt.scatter(self.df[feat2][non_cancer_mask], self.df[feat1][non_cancer_mask],
#                 #            c='green', alpha=0.4, label='Non-cancerous (Feat2)')
            
#             plt.title(f'{feat1} (blue) vs {feat2} (orange)\nRed outline = Cancerous')
