...
def border_irregularity(record: 'Record', k_blobs: int = 3) -> float:
    """
        The function computes the mean combined index M = I * C over the top-k blobs using the following steps:
            1) I = perimeter^2 / (4 * pi * area) -> spikiness measure
                A perfect circle has I = 1, while a more irregular shape has I > 1
            2) C = area_hull / area -> convexity measure
                A perfect convex shape has C = 1, while a more irregular shape has C > 1
            3) M = I * C -> combined index
                So when a blob is both spiky (I > 1) and concave (C > 1) M becomes large
        ...
        Returns:
            float: mean M in [1, ∞), or np.nan if no valid blobs
    ...
    for prop in props:
        ...
        convexity = convex_hull / area

        # Calculate the combined index M = I * C
        M_list.append(irregularity * max(convexity, 1.0)) # Ensure convexity does not exceed 1
